...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        card.innerHTML = `
            ...
            <p class="price">From NPR ${lowestPrice}</p>
...
function buyNow(productId) {
    ...
    // Priority: Explicit toggle -> Keywords in category -> Keywords in name
    const isTopUpCategory = product.useAdvancedUI === true || 
                            category.includes('uc') || 
    ...
    if (isTopUpCategory) {
        // Grid UI (Inspired by ui.jfif)
        let icon = '🎁';
        if (category.includes('uc') || name.includes('uc')) icon = '🎮';
        if (category.includes('diamonds') || name.includes('diamonds')) icon = '💎';
        
        packagesHTML = '<div class="topup-grid">';
        product.packages.forEach((pkg, index) => {
            packagesHTML += `
                <div class="topup-item package-option" data-product-id="${product.id}" data-package-index="${index}">
                    <div class="topup-icon">${icon}</div>
                    <span class="topup-label">${pkg.label}</span>
                    <span class="topup-price">NPR ${pkg.price}</span>
                </div>
            `;
        });
        packagesHTML += '</div>';
    ...
    details.innerHTML = `
        ...
        <h3 class="mt-3">${isTopUpCategory ? 'Select Top-up Amount:' : 'Select Package:'}</h3>
        ${packagesHTML}
    `;
