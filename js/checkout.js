...
let currentCurrency = 'NPR'; // Default to NPR
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(user => {
        if (user) {
            ...
            displayOrderSummary(); 
            displaySelectionCard(); // Show selection at the top
...
function loadRequirementFields() {
    ...
    requirements.forEach(req => {
        ...
        div.innerHTML = `
            <label style="font-weight: 500; margin-bottom: 5px; display: block;">${req}</label>
            <input type="${inputType}" name="requirement_${req.toLowerCase().replace(/\s+/g, '_')}" required placeholder="Enter your ${req}" style="width: 100%; padding: 12px; border: 1px solid var(--color-border); border-radius: 8px; background: rgba(0,0,0,0.02);">
        `;
...
// Display selection card for top-up items
function displaySelectionCard() {
    const card = document.getElementById('selectionCard');
    const display = document.getElementById('selectedPackageDisplay');
    if (!card || !orderItem) return;

    // We only show this for Top-up style items
    const name = (orderItem.productName || '').toLowerCase();
    const isTopUp = name.includes('uc') || name.includes('diamonds');

    if (isTopUp) {
        card.style.display = 'block';
        const icon = name.includes('uc') ? '🎮' : '💎';
        display.innerHTML = `
            <div class="topup-item" style="cursor: default; background: rgba(59, 130, 246, 0.05); border-color: var(--color-primary); width: 100%; max-width: 250px; margin: 0 auto;">
                <div class="topup-icon">${icon}</div>
                <span class="topup-label">${orderItem.package}</span>
                <span class="topup-price">NPR ${orderItem.price}</span>
            </div>
        `;
    }
}
